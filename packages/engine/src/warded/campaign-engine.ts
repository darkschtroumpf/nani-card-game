...
export function createCampaignGame(chapter: ChapterDefinition, save?: CampaignSaveState): GameState {
  ...
  for (const pw of chapter.preplacedWards) {
    ...
    if (loc) {
      ...
      if (emptySlot >= 0) loc.wards[emptySlot] = { ward: pw.ward, isTemporary: false, durability: 4, xp: 0, enhanced: false };
    }
  }
    ...
    switch (effect.type) {
      ...
      case 'bonus_ward': {
        ...
        if (loc) {
          const emptySlot = loc.wards.findIndex(ws => !ws.ward);
          if (emptySlot >= 0) {
            loc.wards[emptySlot] = { ward: effect.wardType, isTemporary: false, durability: 4, xp: 0, enhanced: false };
...
export function migrateSave(save: Partial<CampaignSaveState>): CampaignSaveState {
  ...
  if (!migrated.heroMaxHp) migrated.heroMaxHp = { arlen: 7, arlen_young: 6, jardir: 7, jardir_young: 6, rojer: 7, rojer_young: 5, leesha: 7, leesha_young: 6 } as any;
...
export function createNewSave(): CampaignSaveState {
  return {
    ...
    heroMaxHp: { arlen: 7, arlen_young: 6, jardir: 7, jardir_young: 6, rojer: 7, rojer_young: 5, leesha: 7, leesha_young: 6 },
    wardPowerBonus: 0,
  };
}
